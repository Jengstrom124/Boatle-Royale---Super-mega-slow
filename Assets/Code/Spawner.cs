...
public class Spawner : MonoBehaviour
{
	...
	private void Spawn(int spawnAmount)
	{
		for (int i = 0; i < spawnAmount; i++)
		{
			...
			{
				rb.velocity = new Vector3(Random.Range(-randomThrowAmount, randomThrowAmount), 0,
					Random.Range(-randomThrowAmount, randomThrowAmount));
			}

			if (Random.value > 0.5f)
			{
				rb.angularVelocity = new Vector3(Random.Range(-randomThrowAmount, randomThrowAmount), Random.Range(-randomThrowAmount, randomThrowAmount), Random.Range(-randomThrowAmount, randomThrowAmount));
			}
		}
	}
